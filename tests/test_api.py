...
class TestRESTful:
    ...
    def test_add_api(self,client,entry_list):
        ...
        response = client.post('/api/add',data=json.dumps(data),content_type="application/json", follow_redirects=True)
        # Assert statements
    ...
    def test_prediction(client, entry_list):
        ...
        assert response.status_code == 200
        assert response.headers["Content-Type"] == "application/json"
        assert json.loads(response.get_data(as_text=True))['prediction'] == entry_list[2]